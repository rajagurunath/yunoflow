...
| P6 Durability (DBOS, gated) | ✅ GREEN | `P6_GREEN` | dbos 2.22.0 smoke OK. scheduling/dbos_workflows.py: gated (FEATURE_DBOS default off), lazy import, durable @DBOS.scheduled sweep; wired in lifespan (no-op without flag/extra). Verified: flag-off → 15 tests green (zero regression); flag-on → DBOS launched against Postgres + clean shutdown. dbos kept as [dbos] extra to keep base image lean. |
